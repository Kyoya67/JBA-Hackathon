...
/// @dev time-basedを使う
contract TimeStampBase_DynamicOnChainNFT is ERC721, ERC721URIStorage, Ownable {
    ...
    /// @dev stage設定
    enum Stages {one, two, three, four, five, six, seven, eight}
    ...
    /// tokenIdと現timeをマッピングする変数を定義
    mapping(uint => uint) public growTime;
    ...
    constructor() ERC721("TimeStampBase_DynamicOnChainNFT", "TDO") {}
    ...
    */
    ...
    function nftMint() public onlyOwner{
        ...
        // tokenIdのブロックスタンプ計測開始
        growTime[tokenId] = block.timestamp;

        emit UpdateTokenURI(_msgSender(), tokenId, startFile);
    }

    function growNFT(uint targetId_) public returns (string memory){
        //今のstage
        ...
        //growTimeを今の時間にしてリセット
        growTime[targetId_] = block.timestamp;
    ...
    function curNFT(uint targetId_) public returns (string memory){
        //今のstage
        Stages CurStage = tokenStage[targetId_];

        //次のstageを設定(整数値に型変換)
        uint curStage = uint(CurStage) + 1;
        
        // metaFileの決定
        string memory metaFile = string(abi.encodePacked("data:application/json;base64,",Base64.encode(makeMetaData(curStage, targetId_))));
        _setTokenURI(targetId_, metaFile);
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721, ERC721URIStorage) returns(string memory) {
        if (block.timestamp >= growTime[tokenId] + 1 minutes)
        return growNFT(tokenId);

        else
        return curNFT(tokenId);
